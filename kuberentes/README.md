...
* Primero se instala el cluster que es el [manifiesto](strimzi-kafka-operator/install/cluster-operator/060-Deployment-strimzi-cluster-operator.yaml). Este usa por debajo toda la configuracion que ya viene por default con este operador **Strimzi**
...
2. **Nginx**: Se instala Nginx en un namespace nuevo. 
3. **MetalLB (LoadBalancer)**: Los manifiestos para configurar el LoadBalancer son: [ipadresspool](manifests/ipaddresspool_simple.yaml) y [l2adv](manifests/l2adv_selects_pools_by_list.yaml) para seleccionar la direccion de IP.
